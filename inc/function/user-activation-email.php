...
class UserActivationEmail
{
	...
	public function __construct()
	{
		...
		add_action( 'template_redirect', array( $this, 'update_activation_code_via_url' ) );
	}
	...
	public function check_user_activation_code( $user, $user_login, $password )
	{
		...
		{
			if( $_POST['activation-code'] !== $activation_code or empty($_POST['activation-code']) )
			{
	...
	public function check_user_activation_date($user_id){
				$user = get_userdata( $user_id );
				$current_time	= strtotime('today');
				//$registerday = strtotime($user->user_registered);
				$after72hours = strtotime($user->user_registered . "+3 days");
				if($current_time <  $after72hours){
					return true;
					}else{
					return false;
				}
				
				//return $after72hours .' '. $current_time;
	...
	// My Written Function
	public function update_activation_code_via_url(){
		// if ( is_page() && get_the_ID() == 137 ) {
				global $status, $er;
				$uaekey = $_REQUEST['uae-key'];
				$user_id = $_REQUEST['user_id'];
				if(empty($user_id) or empty($uaekey))
				return false;
				
				
				$activation_code = get_user_meta( $user_id, $this->user_meta, true );
				
				if($activation_code == $uaekey){
				if($this->check_user_activation_date($user_id) == true){
					update_user_meta( $user_id, $this->user_meta, 'active' );
					$status = true;
					}elseif($this->check_user_activation_date($user_id) == false){
					$er = true;
				}
		...
		//$status = $this->check_user_activation_date($user_id);
		// }
...
if ( !function_exists('wp_new_user_notifications') ) :
	 ...
	 */
	function wp_new_user_notifications( $user_id, $plaintext_pass = '' )
	{
